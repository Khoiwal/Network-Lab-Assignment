...
PrintReceivedPacket (Ptr<Socket> socket, Ptr<Packet> packet, Address senderAddress)
{
  ...
  //int socket_id = socket->GetNode()->GetId();
	//double rtime = Simulator::Now().GetSeconds();
  ...
  if (InetSocketAddress::IsMatchingType (senderAddress))
  ...
  else
    {
      oss << " received one packet!";
		...
		//oss<<"\n packets received: "<<packets_received<<"\n  PDR "<<(float)packets_received/(float)packets_send;
		//if(rmap.find(socket_id) != rmap.end()) {
		  //double prev_rtime = rmap[socket_id];
			//double interval = rtime - prev_rtime;
			//double jitter = (double)(interval / 0.250);
			//oss<<"\n Jitter: "<<jitter;
		//}

		//rmap[socket_id] = rtime;
  return oss.str ();
}
...
// Combine try out ...
  ...
  "Bounds", BoxValue (Box (0, 300, 0, 1500, 0, 0)),
...
//	"Z", StringValue ("ns3::UniformRandomVariable[Min=0|Max=10000]"));
  ...
  cout<<"\n\n Enter the number of sinks at receiving side: ";
	cin>>nSinks;
  ...
  // 8. Install FlowMonitor on all nodes ... replaced here ...
...
*/
  ...
  float total_pdr = 0.0;
   ...
   std::cout<<"\n\n Number of sinks: "<<nSinks;
   std::cout<<"\n\n Average jitter :"<<total_jitter / nSinks;
	 std::cout<<"\n\n Average pdr: "<<total_pdr / nSinks;
